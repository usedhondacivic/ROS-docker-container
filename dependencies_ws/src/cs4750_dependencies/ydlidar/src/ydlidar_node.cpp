...
void publish_scan(ros::Publisher *pub,  node_info *nodes,  size_t node_count, ros::Time start, double scan_time, float angle_min, float angle_max, std::string frame_id, std::vector<double> ignore_array, double min_range , double max_range)
{
    ...
    // Fix laser facing wrong direction
    std::rotate(scan_msg.ranges.begin(), scan_msg.ranges.begin()+scan_msg.ranges.size()/2, scan_msg.ranges.end());
    std::rotate(scan_msg.intensities.begin(), scan_msg.intensities.begin()+scan_msg.intensities.size()/2, scan_msg.intensities.end());
